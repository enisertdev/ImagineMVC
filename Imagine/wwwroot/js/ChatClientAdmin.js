...
// Bağlan butonuna tıklanıldığında çalışan olay
document.getElementById("connect-buttonadmin").onclick = async (event) => {
...
connection.on("AdminConnected", (value, client) => {
...
connection.on("ErrorAdminConnection", (value) => {
    alert(value);
    connection.stop();
});

connection.on("AdminMessage", (value) => {
...
connection.on("ClientHasLeftChat", (value) => {
    addMessageAdmin('agentadmin', value);
});
